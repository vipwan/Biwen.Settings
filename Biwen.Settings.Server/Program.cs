...
// Add services to the container.
builder.Services.AddScoped<Biwen.Settings.Server.Services.TestService>();
...
#if DEBUG
    options.ProjectId = $"Biwen.Settings.Server-{"Development"}";
#endif

#if !DEBUG
    options.ProjectId = $"Biwen.Settings.Server-{"Production"}";
#endif
...
//测试服务,访问/test 返回站点名称
app.MapGet("/test", (Biwen.Settings.Server.Services.TestService service) => service.GetSiteName())
    .WithTags(". Test");
